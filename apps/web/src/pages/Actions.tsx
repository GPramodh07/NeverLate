...
export default function Actions({ isDark }: ActionsProps) {
  ...
  const handleRestore = () => {
    setActionsList(displayPending)
    setProcessedActions([])
    triggerToast('Restored all actions for demo purposes!')
  }
  ...
  if (loading) {
    return (
      <div className="pt-20 px-6 pb-12 max-w-[1440px] mx-auto flex flex-col items-center justify-center min-h-[50vh]">
        ...
        <p className={`${isDark ? 'text-zinc-400' : 'text-slate-500'} font-medium animate-pulse`}>Syncing action clearances...</p>
      </div>
    );
  }
  ...
  return (
    <div className="pt-20 px-6 pb-12 max-w-[1440px] mx-auto grid grid-cols-12 gap-6 animate-in fade-in duration-500">
      ...
      {toastMessage && (
        <div className={`fixed bottom-6 right-6 px-5 py-3 rounded-2xl border shadow-xl flex items-center gap-2.5 z-50 animate-in slide-in-from-bottom-5 duration-300 ${isDark
          ...
          <div key={idx} className={`p-5 rounded-2xl border shadow-sm flex items-center space-x-4 hover:-translate-y-0.5 transition-transform duration-200 ${isDark ? 'bg-[#18181b] border-zinc-800' : 'bg-white border-slate-100'
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center justify-between">
                ...
                <span className={`text-[10px] font-bold px-2 py-0.5 rounded ${isDark ? stat.badgeClassDark : stat.badgeClassLight
                  }`}>
                  {stat.badge}
                </span>
              </div>
      ...
      <div className="col-span-12 lg:col-span-8 space-y-6">
          ...
          <div className="flex justify-between items-center mb-6">
            ...
            {actionsList.length === 0 && (
              <button
                ...
                className={`text-[10px] font-bold uppercase tracking-wider underline hover:no-underline ${isDark ? 'text-violet-400' : 'text-purple-600'
                  }`}
              >
          ...
          <div className="space-y-4">
            {actionsList.length === 0 ? (
              <div className="text-center py-12 flex flex-col items-center">
                <div className={`w-14 h-14 rounded-full flex items-center justify-center mb-3 ${isDark ? 'bg-emerald-950/20 text-emerald-400' : 'bg-emerald-50 text-emerald-600'
                  }`}>
              ...
              actionsList.map((action) => {
                ...
                return (
                  <div
                    key={action.id}
                    className={`p-5 rounded-xl border flex flex-col gap-4 transition-all duration-300 transform ${isProcessed
                  ...
                  >
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <span className={`px-2.5 py-0.5 text-[8px] font-extrabold tracking-wider rounded uppercase shrink-0 border ${isDark
                    ...
                    <div className="flex gap-2.5 mt-2 justify-end">
                      <button
                        ...
                        className={`px-4 py-2 border rounded-xl text-[10px] font-bold uppercase tracking-wider transition-all active:scale-[0.98] ${isDark
                      ...
                      <button
                        ...
                        className={`px-4 py-2 text-white text-[10px] font-bold uppercase tracking-wider rounded-xl transition-all active:scale-[0.98] ${isDark
        ...
        <section className={`p-6 rounded-2xl border shadow-sm ${isDark ? 'bg-[#18181b] border-zinc-800' : 'bg-white border-slate-100'
          ...
          <div className="h-48 flex items-end justify-between gap-6 px-4 mt-8 border-b pb-1 dark:border-zinc-800">
            {displayWeeklyChart.map((bar: any, idx: number) => (
        ...
        <section className={`p-6 rounded-2xl border shadow-sm ${isDark ? 'bg-[#18181b] border-zinc-800' : 'bg-white border-slate-100'
          ...
          <div className="space-y-3">
            {[
              ...
              <div
                key={idx}
                className={`p-3 rounded-xl border flex items-center justify-between text-xs transition-colors ${isDark ? 'bg-[#0a0a0c]/80 border-zinc-850' : 'bg-slate-50/50 border-slate-100'
                  }`}
              >
                <div>
