...
export interface Insight {
  id: string;
  type: string; // 'Urgent', 'Interview', 'Action Required', 'Conflict'
...
export function extractInsights(emails: Email[], events: CalendarEvent[]): Insight[] {
...
export function generateMetrics(events: CalendarEvent[], emails: Email[]) {
  ...
  emails.forEach(e => {
    if (e.subject.toLowerCase().includes('urgent') || e.snippet.toLowerCase().includes('deadline')) importantEmails++;
  });
...
export function generateMemory(events: CalendarEvent[], emails: Email[], keepNotes: KeepNote[] = []) {
  const mems = [
...
export function chatHeuristic(query: string, events: CalendarEvent[], emails: Email[]) {
